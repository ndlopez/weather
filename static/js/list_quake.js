...
async function gotdata(){
    ...
    let this_time = zeroPad(tagHeure.getHours()) + ":" + zeroPad(tagHeure.getMinutes());
    ...
    for(let idx = 1;idx < this_info.length; idx++){
        ...
        texty = `<div class='column3 float-left' style='margin:0;border-radius:inherit;'><div class='row-date'><h2 class='col-date float-left'> ${tina.tag} </h2><div class='col-date float-left' style='text-align:left;padding-left:0;'><p><strong> ${days[tina.day]} </strong></p><p><small>${months[tina.monty-1]}</small></p></div></div></div>`;
        ...
        <div class='col-date float-left' style='text-align:left;padding-left:0;'><p></p><p><strong> ${zeroPad(tina.heure)}:${zeroPad(tina.minute)}</p></div><h3 class='col-date float-left'>M${this_info[idx].magnitud}</h3></div></div>`;
